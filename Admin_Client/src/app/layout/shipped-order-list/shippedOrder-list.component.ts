...
@Component({
  ...
  templateUrl: './shippedOrder-list.component.html',
  styleUrls: ['./shippedOrder-list.component.scss']
})
export class shippedOrderListComponent implements OnInit {
  ...
  ngOnInit() {
    this.getPendingOrdersByUserId();
  }


  // Get data from service
  private getPendingOrdersByUserId() {
    // let req_data = Object.assign({ "userId": localStorage.getItuserIdem('userId'), "page": 1 });
    // 6059ae7eb7192f4142140e8e
    this.apiService.getshippiedOrdersByUserId(localStorage.getItem('userId'), this.page)
      .subscribe((result) => {
  ...
  shipNow(data){
    ...
    .then(result=>{
      ...
      if (result["status"] == "SUCCESS") {
        var body = Object.assign({ "data": result });
...
export class ServiceAvailableModalComponent {
  ...
  constructor(public activeModal: NgbActiveModal, private router: Router, private apiService: apiService) { }
  ...
  ngOnInit() {
    ...
    if (this.getBodyData['status'] == 'SUCCESS') {
        ...
        console.log('payment_method', this.getBodyData);
    }
    else {
