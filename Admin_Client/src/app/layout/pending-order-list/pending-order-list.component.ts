...
export class PendingOrderListComponent implements OnInit {
  ...
  viewOrderDetails(item) {
    ...
    if (item && item.order) {
      ...
      modalRef.componentInstance.orderDetails = Object.assign({ "data": item.order });
    }
  }

  private verifyPayment(item) {
    console.log(item);
    let data = {
      "seller_id": localStorage.getItem('userId'),
      // "seller_id": '6059ae7eb7192f4142140e8e',
      "customerId": item.customerId,
      "orderId": item.order._id
    };
    this.apiService.pendingOrderPaymentStatusComplete(data).subscribe(result => {
      if (result['status'] == 'SUCCESS') {
        ...
        this.router.navigate(["/layout/websitependingorders"]);
      }
    });
  }
  ...
  private getPendingOrdersByUserId() {
    ...
    this.apiService.getPendingOrdersByUserId(localStorage.getItem('userId'), this.page)
      .subscribe((result) => {
